...
static ssize_t
piecenew(struct sequence *s, size_t pos, size_t off,
         size_t len)
{
  struct piece *n;
...
static bool
appenddata(struct sequence *s, const uint8_t *data,
           size_t len)
{
  uint8_t *n;
...
void
sequenceprintchangetree(struct sequence *s, char *h,
                        ssize_t c)
{
  char buf[512];
  size_t i;
  ...
  printf("between %zi and %zi ", s->changes[c].prev,
         s->changes[c].next);
  printf("removed from %zi to %zi, ", s->changes[c].rstart,
         s->changes[c].rend);
...
sequenceprint(struct sequence *s)
{
  ...
  for (p = s->pieces[SEQ_start].next; p != SEQ_end;
       p = s->pieces[p].next) {
    printf("[%zi len %zu at %zu]", p, s->pieces[p].len,
           s->pieces[p].pos);
  }
...
size_t
sequencecodepoint(struct sequence *s, size_t pos,
                  int32_t *code)
{
  ssize_t p;
  size_t i, l;
...
size_t
sequenceprevcodepoint(struct sequence *s, size_t pos,
                      int32_t *code)
{
  ssize_t p;
  size_t i;
...
size_t
sequenceindexpos(struct sequence *s, size_t line,
                 size_t col)
{
  size_t i = 0;
  int32_t code;
